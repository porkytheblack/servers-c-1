...
// Function prototypes
void handle_request(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, char *buffer);
...
int main() {
    ...
    while (1) {
        ...
        handle_request(sockfd, &client_addr, addr_len, buffer);
    }
...
void handle_request(int sockfd, struct sockaddr_in *client_addr, socklen_t addr_len, char *buffer) {
    ...
    } else if (create_account(name, acc_type, amount, &acc_no, &pin)) {
        snprintf(response, BUFFER_SIZE, 
               "SUCCESS Account created\nAccount Number: %d\nPIN: %04d\n",
               acc_no, pin);  // %04d ensures 4-digit display
    } else {
...
send_response:
    ...
    int bytes_sent = sendto(sockfd, response, strlen(response), 0,
                          (struct sockaddr *)client_addr, addr_len);
    if (bytes_sent < 0) {
...
int create_account(char *name, char *acc_type, float deposit, int *new_acc_no, int *new_pin) {
    ...
    *new_pin = 1000 + rand() % 9000;
    ...
    new_acc->acc_no = *new_acc_no;  // Store generated number
    new_acc->pin = *new_pin;
